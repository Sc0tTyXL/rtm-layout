...
So that holistically the load is more balanced. You can simply switch the layout.
...
In the OpenTaal dataset 13% of all I and J presses are for IJ; 10% only for pressing the J and 83% for the I. 
I therefor opted to add IJ as a key to the layout. 
This will reduce the usage need for both keys and will greatly encrease the ability to type these words.
...
# Dutch – SCH
The trigram SCH is used frequently in Dutch. Because of the placement of H next to the S in the layout, a chord between the keys allows for SCH to be sent. 
This would save two keypresses. 
...
# Numpad
I currently use a normal numpad layout: 789 456 123 0  
...
Also different itterations for different ocasions:
- Smaller key layout requires the navcluster to be moved
- Putting space on the left actually reduces the layout performance. Placing space on the right side is better
